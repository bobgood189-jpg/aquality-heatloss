...
async function sendSignupOtpEmail(to: string, otp: string, name: string) {
  const greeting = name ? `Здравствуйте, <b style="color:#ECF0FA">${name}</b>!` : 'Здравствуйте!';
  const html = `
<!DOCTYPE html>
<html lang="ru">
...
<body style="margin:0;padding:0;background:#07090F;font-family:'Segoe UI',Arial,sans-serif">
    ...
    <tr><td align="center">
      <table width="520" cellpadding="0" cellspacing="0" style="background:#0D1117;border-radius:16px;border:1px solid rgba(194,112,61,.25);overflow:hidden;max-width:95%">
        ...
        <tr><td style="padding:36px 36px 28px">
          <p style="color:rgba(148,163,184,.7);font-size:14px;margin:0 0 8px">${greeting}</p>
          <p style="color:rgba(148,163,184,.7);font-size:14px;margin:0 0 24px">Для завершения регистрации введите этот код в приложении:</p>
          ...
          <p style="color:rgba(148,163,184,.5);font-size:13px;margin:0 0 8px">⏱ Код действует <b style="color:rgba(148,163,184,.8)">1 час</b></p>
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      to,
      subject: `${otp} — подтверждение регистрации в Aquality WaterPro`,
      html,
    }),
  });
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { email, password, full_name, phone, phone2, city, client_type } = await req.json();

    if (!email || !password) {
      return new Response(JSON.stringify({ error: 'email и password обязательны' }), {
    ...
    // Создаём пользователя через Admin API — без отправки письма от Supabase
    ...
    // Генерируем OTP для подтверждения регистрации
    ...
    if (linkError || !linkData?.properties?.email_otp) {
      console.error('[register] generateLink error:', linkError);
      return new Response(JSON.stringify({ error: 'Не удалось сгенерировать код' }), {
    ...
    if (RESEND_API_KEY) {
      await sendSignupOtpEmail(email, otp, full_name || '');
    } else {
      console.warn('[register] RESEND_API_KEY не задан — пропуск email, otp:', otp);
    }

    return new Response(JSON.stringify({ ok: true, needsConfirm: true }), {
