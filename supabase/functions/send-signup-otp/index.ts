...
async function sendSignupOtpEmail(to: string, otp: string) {
  const html = `
<!DOCTYPE html>
<html lang="ru">
    ...
    <tr><td align="center">
      <table width="520" cellpadding="0" cellspacing="0" style="background:#0D1117;border-radius:16px;border:1px solid rgba(194,112,61,.25);overflow:hidden;max-width:95%">
        ...
        <tr><td style="padding:36px 36px 28px">
          <p style="color:rgba(148,163,184,.7);font-size:14px;margin:0 0 24px">Ваш новый код подтверждения регистрации:</p>
          ...
          <p style="color:rgba(148,163,184,.5);font-size:13px;margin:0">🔒 Никому не передавайте этот код</p>
        </td></tr>
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!RESEND_API_KEY) {
      console.warn('[send-signup-otp] RESEND_API_KEY not set — otp:', otp);
